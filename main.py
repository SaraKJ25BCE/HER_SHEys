...
from integrations import fetch_realtime_jobs, schedule_calendar_meeting
...
def analyze_profile(user: UserProfileRequest):
    try:
        ...
        # 2. Extract top missing skill gap safely
        ...
        # 3. Fetch live job listings via JSearch API
        realtime_jobs = fetch_realtime_jobs(target_role=user.target_role, skill_gap=top_gap)

        return {
            "ai_pipeline": ai_results,
            "matched_returnships": realtime_jobs if realtime_jobs else [],
...
def schedule_meeting(req: ScheduleMeetingRequest):
    """
    Triggers Google Calendar API event creation via Service Account credentials.
    """
    try:
