...
# Fallback text area if no file is uploaded or if extraction is empty
...
if st.button("🚀 Analyze Profile & Generate Roadmap"):
    if not resume_text.strip():
        ...
        with st.spinner("Processing through GenAI engine & DST rules..."):
            payload = {
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    with col1:
                        ...
                        st.write(", ".join(ai["profile"].get("existing_skills", [])))
                    with col2:
                        st.markdown("**Skill Gaps to Work On:**")
                        for gap in ai["profile"].get("missing_skills_for_target", []):
                            st.error(f"• {gap}")
                    ...
                    with c1:
                        ...
                        for job in data.get("matched_returnships", []):
                            ...
                            st.button(f"Apply for {job['title']}", key=job['id'])
                    ...
                    with c2:
                        ...
                        for mentor in data.get("matched_mentors", []):
                            ...
                            st.button(f"Book Session ({mentor['availability']})", key=mentor['id'])
